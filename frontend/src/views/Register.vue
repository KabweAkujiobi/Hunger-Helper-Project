    ...
    <v-form @submit.prevent="register" class="w-50 my-auto mx-10">
      <v-window v-model="step">
        <v-window-item :value="1">
          ...
          <h2 class="text-h4 align-self-center pb-5">Registration</h2>

          <v-select v-model="user.beneficiaryType" variant="outlined" label="Beneficiary Type" :items="beneficiaryTypes"
            item-title="text" item-value="value" required></v-select>
          <v-text-field v-model="user.organisationName" label="Organisation Name" variant="outlined"></v-text-field>
          <v-select v-model="user.organisationType" variant="outlined" label="Organisation Type"
            :items="organisationTypes" item-title="text" item-value="value" required></v-select>
        ...
        <v-window-item :value="2">
          <v-select class="pt-5" v-model="user.donationRole" variant="outlined" label="Donation Role"
            :items="donationRoles" item-title="text" item-value="value" required></v-select>
          <v-text-field v-model.number="user.contactNumber" label="Contact Number" variant="outlined"
            required></v-text-field>
          <v-text-field v-model="user.email" label="Email" variant="outlined" required></v-text-field>
          <v-text-field v-model="user.password" type="string" label="Password" variant="outlined"
            required></v-text-field>

          <v-btn type="submit" color="#dc9333">Register</v-btn>
          <p class="pa-5">If you already have an account <router-link to="/login">Login</router-link></p>
        </v-window-item>
      </v-window>
      <v-card-actions>
        <v-btn v-if="step > 1" color="#dc9333" variant="flat" @click="step--">
          Back
        </v-btn>
        <v-spacer></v-spacer>
        <v-btn v-if="step < 2" color="#dc9333" variant="flat" @click="step++">
          Next
        </v-btn>
      </v-card-actions>
    </v-form>
